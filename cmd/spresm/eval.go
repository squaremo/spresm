...
func evalCmd(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("expected to find spec file %s: %w", specpath, err)
	}
	defer specfile.Close()

	var spec spec.Spec
	if err := yaml.NewDecoder(specfile).Decode(&spec); err != nil {
		return fmt.Errorf("unable to decode spec file at %s: %w", specpath, err)
	}

	nodes, err := eval.Eval(spec)
	if err != nil {
		return fmt.Errorf("unable to evaluate spec file at %s: %w", specpath, err)
	}
